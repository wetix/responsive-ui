<script lang="ts">
  ...
  const onChange = (e: Event | undefined) => {
...
<div style="padding: 2rem">
  <RangeSlider
    min="100"
    max="1000"
    bind:lowerVal={lowSlideValue}
    bind:upperVal={upSlideValue}
  />
  <p>Low: {lowSlideValue}</p>
  <p>Up: {upSlideValue}</p>
  ...
  <button on:click={() => (openModal = true)}>Open Modal</button>
</div>
